...
import ImportGuide, { FileInfoCard } from './ImportGuide'
...
export default function WorkspaceConsole({
  ...
  const menuItems = [
    ...
    { id: 'boundary', icon: '🗺️', label: '行政区划', hint: boundaryData ? `${boundaryData.features?.length || 0} 要素` : '未加载', color: 'rose' },
    { id: 'render', icon: '🎨', label: '渲染控制', hint: '实时调整', color: 'violet' },
  ]
  ...
  return (
    <>
      ...
      <AnimatePresence>
        ...
        {activeMenu === 'import' ? (
          <ModalPanel title="⬆️ 文件导入与波段合成" isOpen onClose={() => setActiveMenu(null)} accentColor="amber">
            <div className="space-y-4">
              ...
              {/* 已导入文件信息 */}
              {(terrainData || overlayData) && (
                <div>
                  <p className="text-xs text-slate-400 mb-2">已导入文件</p>
                  <div className="space-y-2">
                    <FileInfoCard data={terrainData} />
                    <FileInfoCard data={overlayData} />
                  </div>
                </div>
              )}
        ...
        {activeMenu === 'boundary' ? (
          <ModalPanel title="🗺️ 行政区划边界" isOpen onClose={() => setActiveMenu(null)} accentColor="rose">
            <BoundaryPanel
              boundaryData={boundaryData}
              onBoundaryDataChange={onBoundaryDataChange}
              settings={settings}
